 ...
 */
export function setupSW(config) {
    _config = {
    ...
    loadQueue()
        ...
        .catch(() => {
        // اگر IndexedDB در دسترس نبود، صف خالی می‌ماند.
    });
    const swSelf = self;
    ...
    swSelf.addEventListener('install', (event) => {
        ...
        const precachePromise = (async () => {
            ...
            if (!cacheStore || cfg.precache.length === 0)
                return;
            const cache = await cacheStore.open(cfg.cachePrefix + 'precache');
            await Promise.all(cfg.precache.map(async (url) => {
                try {
                    await cache.add(url);
                }
                catch (err) {
    ...
    swSelf.addEventListener('activate', (event) => {
        ...
        const cleanupPromise = (async () => {
            ...
            keepNames.add(cfg.cachePrefix + 'precache');
            for (const route of cfg.routes) {
                if (route.cacheName) {
                    keepNames.add(cfg.cachePrefix + route.cacheName);
                }
            }
            keepNames.add(cfg.cachePrefix + cfg.defaultCacheName);
            keepNames.add(cfg.cachePrefix + cfg.backgroundSyncQueueName);
    ...
    swSelf.addEventListener('fetch', (event) => {
        ...
        if (typeof event.respondWith !== 'function')
            return;
        if (matchedRoute) {
            const cacheName = cfg.cachePrefix + (matchedRoute.cacheName || cfg.defaultCacheName);
            event.respondWith(applyStrategy(matchedRoute.strategy, request, cacheName, matchedRoute.timeout).catch((err) => {
        ...
        const defaultCacheName = cfg.cachePrefix + cfg.defaultCacheName;
    ...
    swSelf.addEventListener('sync', (event) => {
        ...
        if (tag !== cfg.backgroundSyncQueueName)
            return;
 ...
 */
function matchRoute(cfg, request) {
 ...
 */
async function retryWithBackoff(mutation, maxRetries = 3) {
 ...
 */
async function addToQueue(mutation) {
    const db = await openSyncDB();
    if (!db)
        return null;
    return new Promise((resolve) => {
