 ...
 */
function getManagedClasses(el: HTMLElement): string[] {
 ...
 */
function setManagedClasses(el: HTMLElement, classes: string[]): void {
  if (classes.length === 0) {
...
export function processBind(
  ...
  // صرف‌نظر می‌کنیم. این کار setAttributeهای غیرضروری را حذف می‌کند — مخصوصاً
  // وقتی Signal تغییر می‌کند اما مقدار نهایی همان است (مثلاً $count + 0).
  //
