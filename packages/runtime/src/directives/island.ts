...
//                          Useful when you want the placeholder extraction
//                          behavior but not the deferral.
//
// Placeholder support:
//   A child <template slot="placeholder">…</template> is extracted before
//   deferral. Its content is rendered (cloned) into the element so the user
//   sees something while the real content has not been hydrated. When the
//   hydration trigger fires, the placeholder clone is removed and the real
//   children are processed.
//
// SSR safety:
//   - All browser-API access (IntersectionObserver, requestIdleCallback,
//     setTimeout) is guarded with `typeof … !== 'undefined'` checks so the
//     directive is a no-op during server rendering.

/**
 * Signature of the walker-supplied `processChildren` callback.
 */
export type IslandProcessChildren = (
  el: HTMLElement,
...
export function processIsland(
  ...
  hydrateMode: string,
  context: Record<string, any>,
  processChildren: IslandProcessChildren,
): () => void {
  ...
  let placeholderHost: HTMLElement | null = null;
  ...
  if (effectiveMode === 'load') {
    ...
    if (typeof (globalThis as any).requestIdleCallback !== 'undefined') {
      idleHandle = (globalThis as any).requestIdleCallback(() => hydrate());
    } else {
    ...
    if (typeof IntersectionObserver !== 'undefined') {
      ...
      observer.observe(el);
    } else if (typeof (globalThis as any).requestIdleCallback !== 'undefined') {
