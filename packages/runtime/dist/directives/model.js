...
import { isSignal } from '../context.js';
/**
...
function setSignalFromPath(state, path, value, context) {
    ...
    if (context) {
        ...
        const localSignals = context.__zenith_signals__;
    ...
    if (!isSignal(signal)) {
        console.warn(`[zen-model] Cannot find signal for path: ${path}. ` +
            `If this is inside zen-for, use $item (with $ prefix) instead of item.`);
        return false;
    }
    return setSignalValue(signal, resolvedParts, value);
}
/**
 ...
 */
function setSignalValue(signal, parts, value) {
    if (parts.length === 1) {
    ...
    // آبجکت می‌سازد نه یک آرایه. تشخیص نوع و استفاده از spread مناسب.
    ...
    for (let i = 1; i < parts.length - 1; i++) {
        ...
        if (target[key] === null || target[key] === undefined) {
            ...
            // BUG-2 FIX (v1.2.2): nested levels هم array-aware.
 ...
 */
function getInputConfig(el) {
    const tagName = el.tagName;
    const type = el.type;
    if (type === 'checkbox') {
        return {
            eventName: 'change',
            readValue: () => el.checked,
        };
    }
    if (tagName === 'SELECT') {
        return {
 ...
 */
export function processModel(el, expr, context, state) {
    ...
    const eventListener = (event) => {
        // BUG-3 FIX (v1.2.2): stopPropagation حذف شد — باعث می‌شد رویداد به
        // delegated event listeners (document level) نرسد و zen-action روی
        // parentها کار نکند.
