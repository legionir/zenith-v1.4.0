 ...
 *
 * BUG-20 FIX (v1.2.2): لیست کامل دایرکتیوها.
 */
const VALID_DIRECTIVES = new Set([
    ...
    // BUG-20 FIX (v1.2.2): directives جدید.
 ...
 */
function extractExpression(directive, value) {
    if (!value)
        return null;
...
function checkHtmlFile(filePath) {
    ...
    while ((scriptMatch = scriptRegex.exec(stripped)) !== null) {
        ...
        if (scriptContent.includes('eval(') || scriptContent.includes('new Function(')) {
            const idx = scriptMatch.index + scriptContent.indexOf(scriptContent.includes('eval(') ? 'eval(' : 'new Function(');
        ...
        if (expr && (directive === 'zen-text' || directive === 'zen-if' || directive === 'zen-show' ||
            ...
            try {
                ...
                findings.push({
                    ...
                    severity: 'error',
                    rule: 'Z400',
                    message: `Expression syntax error in ${directive}: ${e.message}`,
                });
            }
        }
    ...
    while ((i18nMatch = i18nRegex.exec(stripped)) !== null) {
        ...
        if (seenI18n.has(funcName))
            continue; // فقط یک بار گزارش کن
 ...
 */
export function runCheck(target = '.', options = {}) {
