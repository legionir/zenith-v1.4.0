 ...
 *
 * FIX (v1.2.8): P1-1 — این لیست اکنون از `./security-constants.js` ایمپورت
 * می‌شود تا با Evaluator هماهنگ بماند. قبلاً drift کرده بود و Validator فقط
 * constructor/__proto__/prototype را بلاک می‌کرد.
 */
// FORBIDDEN_PROPERTIES is imported above from ./security-constants.js.
/**
    ...
    switch (node.type) {
        ...
        case 'Identifier':
            if (FORBIDDEN_IDENTIFIERS.includes(node.name)) {
                throw new Error(`Security Alert: Access to '${node.name}' is forbidden. ` +
        ...
        case 'MemberExpression':
            ...
            if (!node.computed && node.property.type === 'Identifier') {
                ...
                if (FORBIDDEN_PROPERTIES.includes(propertyName)) {
                    throw new Error(`Security Alert: Access to property '${propertyName}' is forbidden. ` +
            ...
            if (node.computed && node.property.type === 'Literal') {
                ...
                if (typeof literalValue === 'string' && FORBIDDEN_PROPERTIES.includes(literalValue)) {
                    throw new Error(`Security Alert: Access to property '${literalValue}' via bracket notation is forbidden. ` +
                        `This property can be used for prototype pollution attacks.`);
                }
            }
        ...
        case 'ArrowFunction':
            node.params.forEach(p => {
                // BUG-18 FIX (v1.2.2): check all FORBIDDEN_IDENTIFIERS (not just window/document/eval).
        ...
        // Type Safety
        default:
            throw new Error(`Unknown AST node type during validation: '${node.type}'`);
    }
}
