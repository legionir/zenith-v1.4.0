...
const builtinRules = {
    required: (value) => {
        // FIX (v1.2.3): قبلاً false برای checkbox‌های required قبول می‌شد.
    ...
    url: (value) => {
        ...
        try {
            ...
            return { valid: false, error: 'فرمت URL نامعتبر است', rule: 'url' };
        }
    },
    min: (value, _all, param) => {
        // FIX (v1.2.3): coerce numeric strings.
    ...
    pattern: (value, _all, param) => {
        ...
        // input length at 1000 characters before testing.
    ...
    equals: (value, _all, param) => {
        // FIX (v1.2.3): type coercion for param.
    ...
    creditCard: (value) => {
        ...
        if (digits.length < 13 || digits.length > 19)
            return { valid: false, error: 'شماره کارت نامعتبر', rule: 'creditCard' };
        let sum = 0;
    ...
    nationalId: (value) => {
        ...
        if (code.length !== 10)
            return { valid: false, error: 'کد ملی باید ۱۰ رقم باشد', rule: 'nationalId' };
        if (/^(\d)\1{9}$/.test(code))
            return { valid: false, error: 'کد ملی نامعتبر', rule: 'nationalId' };
        let sum = 0;
        for (let i = 0; i < 9; i++)
...
const asyncRules = new Map();
export function defineRule(name, fn) {
    customRules.set(name, fn);
}
/** ثبت یک قانون async (مثل بررسی uniqueness از سرور). */
export function defineAsyncRule(name, fn) {
 ...
 */
export function getNestedValue(values, path) {
    if (!path.includes('.'))
        return values[path];
 ...
 */
export function setNestedValue(values, path, value) {
    if (!path.includes('.')) {
 ...
 */
export function flattenValues(values, prefix = '') {
    const result = {};
...
export function validateField(value, rulesStr, allValues = {}) {
    if (!rulesStr)
        return { valid: true, error: null };
 ...
 */
export async function validateFieldAsync(value, rulesStr, allValues = {}, signal) {
    if (!rulesStr)
        return { valid: true, error: null };
    const rules = parseRules(rulesStr);
 ...
 */
export async function validateFormAsync(values, rules, signal) {
    const fields = {};
    const flatValues = flattenValues(values);
    for (const [fieldName, rulesStr] of Object.entries(rules)) {
        if (signal?.aborted)
 ...
 */
export async function validateWithLifecycle(values, rules, lifecycle, async = false) {
    // onBeforeValidate
...
export function schemaToFormConfig(schema) {
    const config = {};
...
export function schemaToHtml(schema, formVar = '$form') {
    const lines = [];
    for (const [name, field] of Object.entries(schema.fields)) {
        // FIX (v1.2.3): escape label و placeholder.
