...
export function deserializeState(serializedState: Record<string, any>): Record<string, any> {
  ...
  for (const key in serializedState) {
    ...
    // hasOwnProperty چک می‌کند که key واقعاً روی خود آبجکت باشد (نه روی
    // prototype chain). این از keyهایی مثل `__proto__` جلوگیری می‌کند که
    // اگر در serializedState تزریق شده باشند، می‌توانند prototype آبجکت
    // state (و در نتیجه تمام آبجکت‌های دیگر) را آلوده کنند.
    if (!Object.prototype.hasOwnProperty.call(serializedState, key)) continue;
    // کلیدهای خطرناک `__proto__`, `constructor`, `prototype` را explicit
    // skip می‌کنیم — حتی اگر hasOwnProperty true برگرداند (که در یک
    // آبجکت plain برمی‌گردد، اما دفاع در عمق).
...
export async function hydrate(
  root: HTMLElement,
  initFn?: (state: Record<string, any>) => void,
): Promise<HydrationResult> {
  const startTime = performance.now();

  try {
    const serializedState = (window as any).__ZENITH_STATE__;
    if (!serializedState) {
      return {
        success: false,
        elementsAdopted: 0,
    ...
    const initialRoute = (window as any).__ZENITH_ROUTE__;
 ...
 */
export async function startFromSSR(
  root: HTMLElement,
  initFn?: (state: Record<string, any>) => void,
): Promise<HydrationResult> {
...
export function isHydrationMode(): boolean {
  return typeof window !== 'undefined' && !!(window as any).__ZENITH_STATE__;
}

export function clearHydrationData(): void {
  if (typeof window !== 'undefined') {
    delete (window as any).__ZENITH_STATE__;
    delete (window as any).__ZENITH_ROUTE__;
  }
}
 ...
 */
function skipServerComponents(root: HTMLElement): number {
  ...
  for (let i = 0; i < comments.length; i++) {
    ...
    let current: Node | null = open.nextSibling;
    while (current && current !== close) {
      if (current.nodeType === Node.ELEMENT_NODE) {
        stripped += stripZenAttributes(current as HTMLElement);
      }
 ...
 */
function stripZenAttributes(el: HTMLElement): number {
  let count = 0;
  ...
  for (const child of Array.from(el.children)) {
    count += stripZenAttributes(child as HTMLElement);
  }
  return count;
}
