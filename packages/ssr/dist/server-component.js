 ...
 */
const serverComponentRegistry = new Map();
/**
 ...
 */
export function registerServerComponent(comp) {
 ...
 */
export function getServerComponent(name) {
 ...
 */
export function unregisterServerComponent(name) {
 ...
 * عمدتاً برای تست‌ها.
 */
export function clearServerComponents() {
 ...
 */
export function makeOpenMarker(name) {
    return `<!--${SERVER_COMPONENT_OPEN_PREFIX}${name}-->`;
}
/**
 * ساخت marker comment بست‌شونده.
 */
export function makeCloseMarker(name) {
 ...
 */
export async function renderServerComponent(name, props = {}) {
 ...
 */
export function defineServerComponent(name, render) {
    return { name, render };
}
 ...
 */
function findServerComponentTags(html) {
    const results = [];
 ...
 */
function parseServerComponentAttrs(attrs, state = {}) {
    const props = {};
    let name = '';
    // Attribute regex:
 ...
 */
function resolvePropValue(raw, state) {
 ...
 */
function resolveStateExpression(expr, state) {
    // حذف $ از ابتدا.
    ...
    if (firstKey == null || !(firstKey in state))
        return undefined;
 ...
 */
async function singlePass(html, tags, state) {
    // رندر موازی همه‌ی کامپوننت‌ها.
    const rendered = await Promise.all(tags.map(async (tag) => {
 ...
 */
export async function inlineServerComponents(html, state = {}) {
    let result = html;
    let MAX_PASSES = 10;
    while (MAX_PASSES-- > 0) {
