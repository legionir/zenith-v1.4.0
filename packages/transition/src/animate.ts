...
//
// ── سینتکس ──
//   <div zen-animate="slideUp">…</div>
//   <div zen-animate="bounce,duration:600,easing:ease-in-out">…</div>
//
// ── PRESETS ──
//   یازده پیش‌تنظیم آماده: fadeIn, fadeOut, slideUp, slideDown, slideLeft,
//   slideRight, scaleIn, scaleOut, bounce, shake, rotate.
//
// ── راه‌اندازی Runtime ──
//   `processAnimate` به‌عنوان directive processor توسط walker روی هر عنصری
//   که attribute `zen-animate` داشته باشد فراخوانی می‌شود. این تابع attribute
//   را parse می‌کند، keyframes را از PRESETS برمی‌دارد و `zenAnimate` را
//   صدا می‌زند. مقدار بازگشتی یک no-op dispose است چون انیمیشن خودش پس از
//   پایان با `anim.cancel()` پاکسازی می‌شود.
 ...
 * - delay: 0.
 */
const DEFAULT_ANIMATE_OPTIONS: KeyframeAnimationOptions = {
  duration: 300,
  easing: 'ease',
  fill: 'forwards',
};
 ...
 */
export function parseAnimateAttr(value: string): ParsedAnimateAttr {
  ...
  const options: KeyframeAnimationOptions = { ...DEFAULT_ANIMATE_OPTIONS };
 ...
 */
export function processAnimate(
  el: HTMLElement,
  attrValue: string,
  ...
  let parsed: ParsedAnimateAttr;
  try {
    parsed = parseAnimateAttr(attrValue);
  } catch (err) {
    // preset نامعتبر — هشدار و no-op dispose.
    if (typeof console !== 'undefined' && console.warn) {
      console.warn(`[zen-animate] Failed to parse '${attrValue}':`, (err as Error).message);
    }
    return () => {};
  }
