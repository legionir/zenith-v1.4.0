...
import type { ResourceState } from '@zenith/resource';
...
/**
 * حداقل امضایی که یک شیء باید داشته باشد تا به‌عنوان Resource شناخته شود.
 *
 * ما از `Resource` کلاس `@zenith/resource` مستقیماً import نمی‌کنیم تا این
 * پکیج به‌صورت structural typing با هر شیء مشابه (مثلاً mock در تست‌ها)
 * کار کند. این الگو duck typing است.
 */
interface ResourceLike<T = any> {
  /** Signal وضعیت Resource. */
  readonly signal: Signal<ResourceState<T>>;
  /** متد fetch لیست. برای retry استفاده می‌شود. */
 ...
 */
function isResourceLike(value: any): value is ResourceLike {
  return (
    value !== null &&
    typeof value === 'object' &&
...
/**
 * اطلاعات یک آیتم رندرشده در حالت success.
 */
interface RenderedItem {
  /** نود DOM آیتم. */
  node: HTMLElement;
  /** Signal محلی برای مقدار آیتم ($item). */
  itemSignal: Signal<any>;
  /** Signal محلی برای ایندکس آیتم ($index). */
  indexSignal: Signal<number>;
  /** تابع teardown برای dispose effectهای فرزندان. */
  dispose: () => void;
}
 ...
 */
export function processResourceView(
  el: HTMLElement,
  ...
  const resource = (context as Record<string, any>)[refName];
  ...
  const userTemplate = el.querySelector(':scope > template') as HTMLTemplateElement | null;
  if (!userTemplate) {
    console.warn(
  ...
  // error-text از سوی کاربر قابل override است؛ اگر نباشد، از پیام خود Resource استفاده می‌کنیم.
  ...
  const itemsByKey = new Map<string, RenderedItem>();
  ...
  let slotDispose: (() => void) | null = null;
   ...
   * fallback به index.
   */
  function computeKey(item: any, index: number): string {
   ...
   */
  function renderSuccess(data: any): void {
    clearSlot();
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (existing) {
        ...
        // ─ـ ساخت آیتم جدید ──
        // template.content یک DocumentFragment است. cloneNode(true) یک کپی
        // کامل می‌دهد. اگر template چند top-level element دارد، آنها در یک
        // fragment قرار می‌گیرند — ما آنها را در یک wrapper قرار نمی‌دهیم
        // چون می‌خواهیم ساختار کاربر حفظ شود. اما برای سادگی، فقط firstElementChild
        ...
        const itemSignal = signal<any>(item);
