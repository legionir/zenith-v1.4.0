 ...
 */
export function processAuthView(el, configAttr, context, state) {
    // SSR-safe guard.
    ...
    if (isAuthLike(ref)) {
        ...
        console.warn(`[zen-auth-view] Auth "${configAttr}" not found in context or is neither an Auth instance nor a Signal<AuthState>. ` +
            `Example: Zen.start(app, { auth: createAuth({...}) });`);
        return () => { };
    }
    // ─ـ ۲. استخراج templateهای slot ──
    ...
    if (authenticatedTemplate)
        authenticatedTemplate.remove();
    if (guestTemplate)
        guestTemplate.remove();
    if (!authenticatedTemplate && !guestTemplate) {
        console.warn('[zen-auth-view] No <template slot="authenticated"> or <template slot="guest"> found. ' +
     ...
     */
    function renderSlot(slotName, template) {
